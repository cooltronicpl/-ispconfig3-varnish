...
This ISPConfig 3 plugin is built for those who want to set up Varnish Cache in ISPConfig for WordPress, and CraftCMS sites. We use Varnish for the cache, Apache for the backend, and we added NGINX for SSL termination. Your WordPress site is fast also with WP Rocket. We have some mechanism of automatic Varnish clearing with external plugins like WP Rocket or Proxy Cache Purge on WordPress, and CoolTRONIC.pl Varnish Cache Purge on CraftCMS. This mechanism also enables brotli compression on WordPress with WP Rocket and eaisly respecting of Apache2 .htaccess rules from many WordPress or CraftCMS plugins.
...
* Varnish Cache Purge 1.0.0-dev is now working now with CraftCMS 3.x, now 2.0.0-dev also is testing with external sitemap.xml preload for 4.x or after making changes. This mechanism clears Varnish when we change data on SSL websites or when we disable it on some page (we actually working on this, and we are developing new mechanism for Craft).
...
4. Remove the last string with date of backuped .vhosts files, it must ends with .master. There should be in folder: ```/usr/local/ispconfig/server/conf-custom```, also you can use files from repo when there ware not backuped.
