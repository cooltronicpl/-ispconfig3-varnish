...
This ISPConfig 3 plugin is built for those who want to set up Varnish Cache in ISPConfig for WordPress, and CraftCMS sites. We use Varnish for the cache, Apache for the backend, and we added NGINX for SSL termination. Your WordPress site is fast also with WP Rocket. We have some mechanism of automatic Varnish clearing with external plugins like Proxy Cache Purge on WordPress, and CoolTRONIC.pl Varnish Cache Purge on CraftCMS. This mechanism also enables brotli compression on WordPress with WP Rocket and eaisly respecting of Apache2 .htaccess rules from many WordPress or CraftCMS plugins.
